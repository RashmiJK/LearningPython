...
def write_grayscale(filename,pixels):
    ...
    Args:
        filename: Th name of the BMP file to be created.

        pixels: A rectangular image stored as a sequence of rows.
            Each row must be an iterable series of integers in the range --255.
    ...
    with open(filename, 'wb') as bmp:
        ...
        bmp.write(b'\x00\x00\x00\x00')  # to the pixel data.Zero place holder for now.
        ...
        # Fill in filesize place holder
        bmp.seek(size_bookmark)
        bmp.write(_int32_to_bytes(eof_bookmark))

        # Fill in pixel offset place holder
...
def dimesions(filename):
    """ Determine the dimesions in pixels of a BMP image.
